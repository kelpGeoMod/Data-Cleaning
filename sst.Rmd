...
new_names <- paste0(month.abb, "-", rep(2014:2015, each=12))
...
# Create vector of year-month strings
year_month <- expand.grid(year=2014:2015, month=1:12) %>% 
  mutate(month_name = month_names[month])

# Set names of RasterBrick
names(raster_brick) <- paste0(year_month$year, "-", year_month$month_name)
```
...
plot_raster <- function(x, name) {
  plot(x, main = "", col = colorRampPalette(c("blue", "white", "red"))(100), 
       xlim = extent(-180, 180, -90, 90))
  title(main = paste0(format(name, "%b-%Y"), " - ", names(x)), 
        font.main = 1, cex.main = 1.5)
}

# Create the GIF
saveGIF({
  for (i in 1:length(raster_stack_list)) {
    plot_raster(raster_stack_list[[i]], names(raster_stack_list[[i]]))
  }
}, movie.name = "raster_stack.gif", interval = 0.5, ani.width = 800, ani.height = 400)
...
# Set names of layers in raster brick
names(raster_brick) <- format(as.Date(paste0("01-", names(raster_brick)), format = "%d-%b-%Y"), "%b-%Y")

# Create animation
ani.options(interval = 0.2, ani.width = 600, ani.height = 500)
saveGIF({
  for(i in 1:nlayers(raster_brick)) {
    plot(raster_brick[[i]], main = names(raster_brick)[i], legend = FALSE, col = rev(terrain.colors(10)))
  }
}, movie.name = "kelp_growth.gif", renderer = gifski_renderer())
